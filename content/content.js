...
function processShadowDOM(shadowRoot) {
  const threads = shadowRoot.querySelectorAll(
...
function processThreadHost(threadHost) {
  ...
  if (commentEl?.shadowRoot) {
    const richText = commentEl.shadowRoot.querySelector(
...
function processReplyHost(replyHost) {
  ...
  const richText = replyHost.shadowRoot.querySelector(
    "#main > bili-rich-text"
  );
  if (richText?.shadowRoot) {
...
function applyLegacyHide(element) {
  const container = element.closest(
    ".root-reply-container, .sub-reply-container, .reply-wrap"
  );
  if (container) {
      ...
      for (const node of mutation.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          if (
            ...
            node.querySelector?.("bili-comments")
          ) {
